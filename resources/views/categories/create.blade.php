<div>
    <x-header title="{{ __('app.categories.create') }}" separator progress-indicator>
        <x-slot:actions>
    ...
    <x-card shadow class="bg-base-100 max-w-2xl mx-auto">
        <x-form wire:submit="save">
            ...
            <x-slot:actions>
                <x-button label="{{ __('app.common.cancel') }}" link="{{ route('categories.index') }}" />
                <x-button label="{{ __('app.common.create') }}" type="submit" class="btn-primary" icon="o-check" spinner="save" />
            </x-slot:actions>
        </x-form>
    </x-card>
</div>